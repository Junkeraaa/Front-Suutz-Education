...
const LoginScreen = () => {
  ...
  const handleLogin = async () => {
    const url = loginAsTeacher
      ? 'http://srv656114.hstgr.cloud:3000/api/auth/register/teacher'
      : 'http://srv656114.hstgr.cloud:3000/api/auth/register/customer';
      ...
      if (response.ok) {
        ...
        if(dataRole == 'student'){
          try {
            const response = await axios.post(`http://srv656114.hstgr.cloud:4000/wallet/create/${alunoId}`);
